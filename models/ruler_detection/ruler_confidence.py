...
# --------------------------------------------------------------------------- #
THRESHOLDS = {
    "snr_db":          (6.3617, 1.7858),    # nettete du pic
    "phase_coherence": (0.8548, 0.2485),   # alignement de la regle
    "support_frac":    (0.0281, 0.0055),   # fraction de lignes concernees
}
...
def _hann(n):
    """Fenetre de Hann mise en cache (elle ne depend que de la longueur)."""
    w = np.hanning(n)
...
@lru_cache(maxsize=16)
def _freqs(n):
    """Axe des frequences mis en cache."""
    f = np.fft.rfftfreq(n)
...
@lru_cache(maxsize=16)
def _fenetres_median(n, stride=8):
    """Indices des fenetres du median glissant, mis en cache.

    Ces indices ne dependent que de la longueur du spectre : ils sont donc
    identiques pour toutes les lignes d'une meme image, et calcules une fois.
    Le clip aux bornes remplace le padding "edge" sans rien allouer.
    """
    k = max(11, n // 16)
    if k % 2 == 0:
        k += 1
...
def _noise_floor(mag):
    """Niveau de bruit local du spectre (median glissant, echantillonne).
    ...
    On ne calcule la mediane qu'une position sur 8, puis on interpole : le
    plancher varie lentement avec la frequence, donc le resultat est
    indiscernable d'un median glissant complet, pour bien moins de calcul.
    """
    idx, half, centres, grille = _fenetres_median(len(mag))
    med = np.partition(mag[idx], half, axis=1)[:, half]
    return np.interp(grille, centres, med) + 1e-12
...
def phases_at(rows_pixels, f):
    """Phases de PLUSIEURS lignes a une meme frequence f, en un seul produit.
    ...
    rows_pixels : tableau (n_lignes, N). L'exponentielle complexe est construite
    une seule fois et partagee, au lieu d'etre refaite pour chaque ligne.
    """
...
def phase_at(row, f):
    """Phase d'une seule ligne (conserve pour compatibilite)."""
...
def phase_coherence(rows, phases, n_slopes=1024):
    ...
    Tester toutes les pentes revient exactement a faire une FFT des phases
    placees sur l'axe des lignes : on obtient les memes pentes, en O(M log M)
    au lieu de O(M x n_lignes).
    """
    n = len(rows)
    if n < 6:
        return 0.0
    ...
    # taille de FFT : puissance de 2, au moins l'etendue des lignes et la
    # resolution de pente demandee (pas de repliement possible)
    M = 1 << int(max(n_slopes, r.max() + 1) - 1).bit_length()

    grille = np.zeros(M, dtype=complex)
    np.add.at(grille, r, np.exp(1j * np.asarray(phases, dtype=float)))
    best = float(np.abs(np.fft.fft(grille)).max() / n)
...
def calibrate_from_annotations(json_path, images_root=None, ratio=5,
    ...
    print(n_illisibles, "images illisibles")
